...
// src/index.ts
var getIconSvg = (name, style = "outline") => {
  const icon = IconRegistry.get(name);
  if (!icon) return void 0;
  return icon.variants?.[style] || icon.svg;
};
var getAllIconNames = () => {
  return IconRegistry.getAll().map((icon) => icon.name);
};

export { IconRegistry, getAllIconNames, getIconSvg, homeIcon, settingsIcon, userIcon };
